...
export default function TestPage(props){
  ...
  const [head_movement_detected, setHeadMovementDetected] = useState(false);
  const [checkedPrevLogs, setCheckedPrevLogs] = useState(false);
  const [face_visible, setFaceVisible] = useState(true); // NEW: Track if face is detected
  
  const history = useHistory();

  /**
   * The below 5 functions are helper functions to set state
  ...
  // NEW: Function to handle head movement detection from Posenet
  function update_head_movement(){
    setHeadMovementDetected(true);
  }

  // NEW: Function to receive face detection state from Object_Detection
  function handleFaceDetectionChange(isFaceDetected){
    setFaceVisible(isFaceDetected);
  }
  ...
  function sendLogsToServer(){
    axios.post('/api/logs/update',{
          ...
          head_movement_detected: head_movement_detected, // NEW: Add head movement to logs
      })
  ...
  function getPreviousLogs(){
      ...
      .then(function (response) {
          ...
          setHeadMovementDetected(response.data.head_movement_detected || false); // NEW
      })
      .catch(function (err) {
          console.log(err);
      });
  ...
  function handleKeyPress(event){
      ...
      if (event.altKey) {
          setKeyPress(prevKeyPress => prevKeyPress + 1);
  ...
  }, [minutes, key_press, tab_change, mobile_phone_found, prohibited_object_found, face_not_visible, multiple_faces_visible, head_movement_detected, exam_id, student_name, student_email]);
  ...
  return (
      <div style={{ height: "100%"}} className="my_container" id="my_container">
        ...
        {/* Object Detection Component with face detection callback */}
        <div className="detect">
          ...
          {/* Posenet Component for head movement detection - receives face detection state */}
          <Posenet 
            onHeadMovement={update_head_movement} 
            faceDetected={face_visible}
          />
        </div>
        ...
        <div className="time_rem">
          <p><h1 style={{ fontSize: '25px', margin: '0', fontWeight: 'bold', marginLeft: '150px'}}>Timer:</h1>{minutes === 0 && seconds === 1 ? null : <h1 align="center" style={{ fontSize: '69px' }}>  {minutes}:{seconds < 10 ? `0${seconds}` : seconds}</h1>
          } </p>
        </div>
